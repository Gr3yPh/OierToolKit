/*
 * OierToolKit (otk) - 一个为 Oier 量身定制的轻量级 Linux 本地评测姬与题目管理工具。 
...
###############################################################################
    ...
    try {
      ...
      while (true) {
        ...
        switch (cmd) {
          ...
          case "show":
        if (tokens.length < 2) {
            System.out.println(YELLOW + "用法: show w (免责声明) 或 show c (许可证)" + RESET);
        } else {
            String subCmd = tokens[1].toLowerCase();
            if (subCmd.equals("w")) {
                System.out.println("OierToolKit 版权所有 (C) 2026 魇珩Gr3yPh4ntom");
                System.out.println("本程序不提供任何担保，甚至不保证适用于特定目的。");
                System.out.println("详情请参阅 GNU 通用公共许可证。");
            } else if (subCmd.equals("c")) {
                System.out.println("这是一个自由软件，欢迎您在 GPLv3 协议下重新分发它。");
                System.out.println("在特定条件下，您可以修改并共享它。");
                System.out.println("完整许可证全文请查看根目录下的 LICENSE 文件。");
            } else {
                System.out.println(RED + "错误: 未知的 show 参数 '" + tokens[1] + "'，请输入 'show w' 或 'show c'" + RESET);
            }
        }
        break;
          default:
  ...
  private static void printAuthor() {
    System.out.println("OierToolKit v0.8 by 魇珩Gr3yPh4ntom\n" +
  ...
  private static void printHelp() {
    ...
    System.out.println("  set [key] [value]    - 设置当前项目的配置 (支持 time / memory / version / o2)");
    ...
    System.out.println("  show w               - 显示软件免责声明 (Warranty)");
    System.out.println("  show c               - 显示软件复制与分发条件 (Copying/License)");
    ...
    if (currentProject.isEmpty()) {
      ...
      if (Files.exists(iniPath)) {
        ...
        try (FileInputStream fis = new FileInputStream(iniPath.toFile())) {
          ...
          System.out.println("  version      : "+props.getProperty("version","c++98"));
          System.out.println("  o2           : "+props.getProperty("o2","1"));
        } catch (Exception e) {
  ...
  private static void handleSetCommand(String[] tokens) {
    if (tokens.length < 3) {
      System.out.println(YELLOW + "用法: set time/memory/version/o2 [VALUE]" + RESET);
      return;
    }
    ...
    try {
      ...
      else if(key.equals("version") || key.equals("ver"))
        props.setProperty("version",value);
      else if(key.equals("o2"))
        props.setProperty("o2",value);
      else {
        System.out.println(YELLOW + "未知配置项: " + key + RESET);
        return;
      }
  ...
  private static void createSample(LineReader reader) {
    ...
    System.out.println("请输入样例输入 (换行后发送EOF以结束):");
    String inputData = readUntilEOF(reader);

    System.out.println("请输入样例输出 (换行后发送EOF以结束):");
  ...
  private static void runTest() {
    ...
    try {
      Path iniPath = currentDir.resolve(currentProject + ".ini");
      FileInputStream fis = new FileInputStream(iniPath.toFile());
      Properties props = new Properties();
      props.load(fis);
      String cppVersion=props.getProperty("version","c++17");
      Integer o2Switch=Integer.parseInt(props.getProperty("o2","1"));
      List<String> command = new ArrayList<>();
      command.add("g++");
      command.add("-std=" + cppVersion);
      if(o2Switch==1) command.add("-O2");
      command.add(cppPath.toString());
      command.add("-o");
